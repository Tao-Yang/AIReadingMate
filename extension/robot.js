/* 阅读伴侣机器人（扩展版）——与网页版一致的琥珀身小机器人，Three.js 基本体搭建。
 * 在内容脚本隔离环境运行；three.min.js 先注入并暴露全局 THREE。
 * 对外暴露 window.Robot：init(canvas) / setState('idle'|'thinking'|'happy')
 */
if (!window.Robot) (function () {
  const COLORS = {
    head: 0xeef1f5,
    headDark: 0xd6dbe3,
    amber: 0xf2a52b,
  ...
  let head, eyeL, eyeR, leftArm, rightArm;
  ...
  function animate() {
    ...
    if (state === "thinking") {
      leftArm.rotation.z = Math.sin(t * 7) * 0.5;
      ...
      leftArm.rotation.z = 0.9 + bounce * 0.4;
      ...
      if (t > stateUntil) setStateInternal("idle");
    } else {
  ...
  window.Robot = {
    init(canvas) {
      if (boundCanvas === canvas && renderer) return; // 已初始化
      boundCanvas = canvas;
